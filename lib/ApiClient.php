...
class ApiClient
{
     ...
     */
    public function __construct(\MessageMedia\RESTAPI\Configuration $config = null)
    {
        if ($config == null) {
     ...
     *
     * @throws \MessageMedia\RESTAPI\ApiException on a non 2xx response
     * @return mixed
     */
    public function callApi($resourcePath, $method, $queryParams, $postData, $headerParams, $responseType = null, $endpointPath = null)
    {
        ...
        if ($postData and in_array('Content-Type: application/x-www-form-urlencoded', $headers)) {
            ...
            $postData = json_encode(\MessageMedia\RESTAPI\ObjectSerializer::sanitizeForSerialization($postData));
        }

        $url    = $this->config->getHost() . $resourcePath;
        $key    = $this->config->getApiKey('key');
        $secret = $this->config->getApiKey('secret');

        $date = gmdate('D, d M Y H:i:s T', time());
        $requestLine = $method . ' ' . parse_url($this->config->getHost())['path'] . $resourcePath . ' HTTP/1.1';
        $content = 'date: ' . $date . "\n" . 'content-md5: ' . md5($postData) . "\n" . $requestLine;
        $signature = base64_encode(hash_hmac('sha1', $content, $secret, true));

        $headers[] = 'Authorization: hmac username="' . $key . '", algorithm="hmac-sha1", headers="date content-md5 request-line", signature="' . $signature . '"';
        $headers[] = 'Date: ' . $date;
        $headers[] = 'Content-MD5: ' . md5($postData);
